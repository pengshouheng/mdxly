...
KeyOld::KeyOld(int pin, uint8_t input)
{
...
bool KeyOld::read(bool sta) // return the value as degrees
{
...
bool KeyOld::read(bool sta, int min, int max) // return the value as degrees
{
  // calculate and store the values for the given channel
  int8_t channel = keys[this->keyIndex].Pin.nbr;
  if ( channel < NUM_DIGITAL_PINS )  // ensure channel is valid
